...
const LoaiKhoDialog: React.FC<LoaiKhoDialogProps> = ({ isOpen, onClose, onSuccess, initialData }) => {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const data = {
      ma_loai_kho: formData.get('ma_loai_kho') as string,
      ten_kho: formData.get('ten_kho') as string,
    };
  ...
  return createPortal(
    <div className="fixed inset-0 z-9999 flex items-center justify-center p-4">
      ...
      <div 
        className={clsx(
          "relative w-full max-w-[500px] bg-white shadow-2xl rounded-2xl flex flex-col transition-all duration-300",
      ...
      >
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-[17px] font-bold text-slate-900 leading-tight">
                {initialData ? 'Cập nhật phân loại kho' : 'Thêm phân loại kho mới'}
              </h2>
              <p className="text-[11px] text-slate-500 font-medium tracking-tight">Khai báo mã định danh và tên gọi mục đích sử dụng kho</p>
            </div>
          </div>
          <button
            onClick={handleClose}
        ...
        <form id="loaikho-form" onSubmit={handleSubmit} className="flex-1 overflow-y-auto no-scrollbar bg-[#f8fafc]/50">
          <div className="p-6 space-y-6">
            ...
            <div className="bg-white p-5 rounded-2xl border border-slate-200/60 shadow-sm space-y-5">
              <div className="flex items-center gap-2 mb-2">
                ...
                <h3 className="text-[12px] font-bold text-purple-600 uppercase tracking-wider">Thông tin phân loại</h3>
              </div>
