...
interface ThanhToanXuatDialogProps {
  isOpen: boolean;
  onClose: () => void;
  onSuccess: () => void;
  initialData?: any;
...
const ThanhToanXuatDialog: React.FC<ThanhToanXuatDialogProps> = ({ isOpen, onClose, onSuccess, initialData }) => {
      ...
      >
        ...
        <div className="px-6 py-4 border-b border-slate-200 flex items-center justify-between bg-white sticky top-0 z-10 shrink-0">
          <div className="flex items-center gap-3">
            <div className="size-10 rounded-xl bg-pink-500/10 flex items-center justify-center text-pink-600 shadow-sm">
              <HandCoins size={22} />
            </div>
            <div>
              <h2 className="text-[17px] font-bold text-slate-900 leading-tight">
                {initialData ? 'Cập nhật thu tiền' : 'Ghi nhận thu tiền mới'}
              </h2>
              <p className="text-[11px] text-slate-500 font-medium tracking-tight">Đối soát và thu hồi nợ từ khách hàng</p>
            </div>
          </div>
          <button
            onClick={handleClose}
        ...
        {/* Body */}
        <form id="payxuat-form" onSubmit={handleSubmit} className="flex-1 overflow-y-auto no-scrollbar bg-[#f8fafc]/50">
          <div className="p-6 space-y-6">
            ...
            <div className="bg-white p-5 rounded-2xl border border-slate-200/60 shadow-sm space-y-5">
              <div className="flex items-center gap-2 mb-2">
                <div className="size-6 rounded-md bg-pink-500/10 flex items-center justify-center text-pink-600">
                  <Hash size={14} />
                </div>
                <h3 className="text-[12px] font-bold text-pink-600 uppercase tracking-wider">Chứng từ thu tiền</h3>
              </div>
              ...
              <div className="grid grid-cols-1 gap-5">
                ...
                <div className="space-y-1.5">
                  ...
                  <div className="relative">
                    <select
                      ...
                      className="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-[14px] font-bold text-slate-900 focus:ring-2 focus:ring-pink-500/20 focus:border-pink-500 outline-none transition-all appearance-none cursor-pointer"
                    >
                ...
                <div className="space-y-1.5">
                  <label className="flex items-center gap-2 text-[13px] font-bold text-slate-700">
                    <DollarSign size={14} className="text-slate-400" /> Số Tiền Thu (VND) <span className="text-red-500 font-bold">*</span>
                  </label>
                  <input
                    type="number"
                    name="so_tien"
                    ...
                    placeholder="0"
                    className="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-[14px] font-bold text-slate-900 focus:ring-2 focus:ring-pink-500/20 focus:border-pink-500 outline-none transition-all placeholder-slate-400 font-mono"
                  />
                </div>
              </div>
            </div>
...
export default ThanhToanXuatDialog;
