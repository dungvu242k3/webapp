...
import ThanhToanXuatDialog from './components/ThanhToanXuatDialog';

interface ThanhToanXuatData {
  id: string;
...
const ThanhToanXuatPage: React.FC = () => {
  const navigate = useNavigate();
  const [searchTerm, setSearchTerm] = useState('');
  const [currentTime, setCurrentTime] = useState(new Date());
  const [data, setData] = useState<ThanhToanXuatData[]>([]);
  const [loading, setLoading] = useState(true);
  const [isDialogOpen, setIsDialogOpen] = useState(false);
  const [selectedItem, setSelectedItem] = useState<ThanhToanXuatData | null>(null);
  ...
  const fetchThanhToan = async () => {
    ...
    try {
      ...
      console.error('Lỗi khi tải danh sách thu tiền:', err);
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleDelete = async (id: string) => {
    if (!window.confirm('Bạn có chắc chắn muốn xóa bản ghi thu tiền này?')) return;
    try {
      const { error } = await supabase.from('thanh_toan_xuat').delete().eq('id', id);
      if (error) throw error;
      fetchThanhToan();
    } catch (err) {
  ...
  const handleEdit = (item: ThanhToanXuatData) => {
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-left border-collapse">
              ...
              <tbody className="divide-y divide-slate-100">
                {loading ? (
                  <tr>
                    <td colSpan={5} className="px-6 py-12 text-center text-slate-400 font-medium italic">Đang tải lịch sử thu tiền...</td>
                  </tr>
                ) : filteredData.length === 0 ? (
                  <tr>
                    <td colSpan={5} className="px-6 py-12 text-center text-slate-400 font-medium italic">Chưa có dữ liệu thu tiền nào</td>
                  </tr>
                ) : (
...
export default ThanhToanXuatPage;
