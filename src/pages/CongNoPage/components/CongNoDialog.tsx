...
const CongNoDialog: React.FC<CongNoDialogProps> = ({ isOpen, onClose, onSuccess, initialData }) => {
  ...
  return createPortal(
    <div className="fixed inset-0 z-9999 flex items-center justify-center p-4">
      ...
      >
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-[17px] font-bold text-black leading-tight">
                {initialData ? 'Cập nhật phiếu nợ' : 'Lập phiếu nợ mới'}
              </h2>
              <p className="text-[11px] text-slate-900 font-medium">Vui lòng điền thông tin tham chiếu và số tiền</p>
            </div>
          </div>
          <button
            onClick={handleClose}
        ...
        {/* Body */}
        <form id="congno-form" onSubmit={handleSubmit} className="flex-1 overflow-y-auto no-scrollbar bg-[#f8fafc]/50">
          <div className="p-6 space-y-6">
            ...
            <div className="bg-white p-5 rounded-2xl border border-slate-200/60 shadow-sm space-y-5">
              <div className="flex items-center gap-2 mb-2">
                ...
                <h3 className="text-[12px] font-bold text-amber-600 uppercase tracking-wider">Thông tin phiếu nợ</h3>
              </div>

              <div className="grid grid-cols-2 gap-5">
                <div className="space-y-1.5 col-span-2">
                  <label className="flex items-center gap-2 text-[13px] font-bold text-slate-700">
                    <Tag size={14} className="text-slate-400" /> ID Tham chiếu (Mã HĐ, ĐH...) <span className="text-red-500 font-bold">*</span>
                  </label>
                  <input
                    type="text"
                    name="id_ref"
                    ...
                    placeholder="Nhập mã đơn hàng hoặc hợp đồng..."
                ...
                <div className="space-y-1.5">
                  <label className="flex items-center gap-2 text-[13px] font-bold text-slate-700">
                    <DollarSign size={14} className="text-slate-400" /> Số tiền nợ <span className="text-red-500 font-bold">*</span>
                  </label>
                ...
                <div className="space-y-1.5">
                  <label className="flex items-center gap-2 text-[13px] font-bold text-slate-700">
                    <Clock size={14} className="text-slate-400" /> Thời gian ghi nhận
                  </label>
                  <input
                    type="datetime-local"
                    name="thoi_gian"
                    defaultValue={initialData?.thoi_gian ? new Date(initialData.thoi_gian).toISOString().slice(0, 16) : new Date().toISOString().slice(0, 16)}
                    className="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-[14px] font-bold text-slate-900 focus:ring-2 focus:ring-amber-500/20 focus:border-amber-500 outline-none transition-all"
                  />
                </div>
              </div>
            </div>
            ...
            <div className="bg-white p-5 rounded-2xl border border-slate-200/60 shadow-sm space-y-5">
              <div className="flex items-center gap-2 mb-2">
                ...
                <h3 className="text-[12px] font-bold text-amber-600 uppercase tracking-wider">Ghi chú kèm theo</h3>
              </div>

              <div className="space-y-1.5">
                <label className="flex items-center gap-2 text-[13px] font-bold text-slate-700">
                  <MessageSquare size={14} className="text-slate-400" /> Nội dung ghi chú
                </label>
                <textarea
                  name="ghi_chu"
                  rows={3}
                  defaultValue={initialData?.ghi_chu || ''}
                  placeholder="Nhập lý do nợ hoặc thông tin bổ sung..."
                  className="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-[14px] font-bold text-slate-900 focus:ring-2 focus:ring-amber-500/20 focus:border-amber-500 outline-none transition-all resize-none placeholder-slate-400"
                />
              </div>
            </div>
          </div>
        ...
        <div className="px-6 py-4 border-t border-slate-200 bg-white flex items-center justify-between sticky bottom-0 z-10">
          ...
          <button
            ...
            form="congno-form"
            disabled={loading}
            className="px-8 py-2.5 bg-amber-500 text-white rounded-xl text-[14px] font-bold hover:bg-amber-600 shadow-lg shadow-amber-500/20 flex items-center gap-2.5 active:scale-95 transition-all disabled:opacity-50 disabled:cursor-not-allowed"
          >
            <Plus size={20} />
