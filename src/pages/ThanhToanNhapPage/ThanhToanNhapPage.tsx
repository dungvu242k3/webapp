...
import ThanhToanNhapDialog from './components/ThanhToanNhapDialog';

interface ThanhToanNhapData {
  id: string;
...
const ThanhToanNhapPage: React.FC = () => {
  const navigate = useNavigate();
  const [searchTerm, setSearchTerm] = useState('');
  const [currentTime, setCurrentTime] = useState(new Date());
  const [data, setData] = useState<ThanhToanNhapData[]>([]);
  const [loading, setLoading] = useState(true);
  const [isDialogOpen, setIsDialogOpen] = useState(false);
  const [selectedItem, setSelectedItem] = useState<ThanhToanNhapData | null>(null);
  ...
  const handleDelete = async (id: string) => {
    if (!window.confirm('Bạn có chắc chắn muốn xóa bản ghi thanh toán này?')) return;
    try {
      const { error } = await supabase.from('thanh_toan_nhap').delete().eq('id', id);
      if (error) throw error;
      fetchThanhToan();
    } catch (err) {
  ...
  const handleEdit = (item: ThanhToanNhapData) => {
    ...
    <div className="min-h-screen bg-[#f1f5f9] font-display text-slate-700">
        ...
        <div className="max-w-full mx-auto px-4 h-[60px] flex items-center justify-between">
          <div className="flex items-center gap-6">
            ...
            <nav className="flex items-center gap-3 text-[13px]">
              ...
              <ChevronRight size={14} className="text-slate-300" />
              <span className="bg-[#2563eb] text-white px-2.5 py-0.5 rounded-md font-medium text-[13px]">Thanh toán nhập</span>
            </nav>
          </div>
      ...
      <main className="max-w-full mx-auto p-4 space-y-4">
        ...
        <div className="flex gap-2">
          ...
          >
            <Folder size={18} /> Nhật ký thanh toán nhập hàng
          </button>
        </div>
        ...
        <div className="bg-white p-3 rounded-lg border border-slate-200 shadow-sm flex items-center justify-between gap-4">
          <div className="flex items-center gap-3 flex-1">
            ...
            <div className="relative w-[300px]">
              ...
              <input
                ...
                placeholder="Tìm mã thanh toán, số phiếu..."
                value={searchTerm}
          ...
          <div className="flex items-center gap-3">
            ...
            <button
              onClick={handleAdd}
              className="bg-green-600 hover:bg-green-700 text-white px-5 py-1.5 rounded flex items-center gap-2 text-[14px] font-semibold transition-all active:scale-95 shadow-lg shadow-green-500/10"
            >
              <Plus size={20} /> Lập thanh toán
            </button>
          </div>
        </div>
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="bg-slate-50 border-b border-slate-200 text-slate-600 text-[11px] font-bold uppercase tracking-wider">
                  ...
                  <th className="px-4 py-3 w-[150px]">ID Thanh toán</th>
                  <th className="px-4 py-3">Phiếu Nhập Gốc (ID REF)</th>
              ...
              <tbody className="divide-y divide-slate-100">
                {loading ? (
                  <tr>
                    <td colSpan={5} className="px-6 py-12 text-center text-slate-400 font-medium italic">Đang tải lịch sử thanh toán...</td>
                  </tr>
                ) : filteredData.length === 0 ? (
                  <tr>
                    <td colSpan={5} className="px-6 py-12 text-center text-slate-400 font-medium italic">Chưa có dữ liệu thanh toán nào</td>
                  </tr>
                ) : (
                    ...
                    <tr key={item.id} className="hover:bg-slate-50/80 transition-colors group">
                      ...
                      <td className="px-4 py-4">
                        <div className="flex items-center gap-2.5 text-slate-900 font-bold">
                           ...
                           {item.nhap_hang?.so_phieu || 'Chưa gán'}
                           <span className="text-[10px] text-slate-300 font-mono ml-2">({item.id_ref})</span>
                        </div>
                      </td>
                      <td className="px-4 py-4 text-right">
                        <div className="flex items-center justify-end gap-2 text-green-600 font-bold tabular-nums">
      ...
      <ThanhToanNhapDialog 
        ...
        onSuccess={fetchThanhToan}
      />
    </div>
  );
};

export default ThanhToanNhapPage;
