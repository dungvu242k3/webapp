...
interface ThanhToanNhapData {
  ...
  ma_thanh_toan: string; // This is the user-friendly ID
  id_ref: string;
  so_tien: number;
  id_nhap?: string; // Flattened from nhap_hang relation
}
...
const ThanhToanNhapPage: React.FC = () => {
  ...
  const fetchThanhToan = async () => {
    ...
    try {
      ...
      const flattenedData = (res || []).map((item: any) => ({
        ...item,
        id_nhap: Array.isArray(item.nhap_hang) ? item.nhap_hang[0]?.id_nhap : item.nhap_hang?.id_nhap
      }));
  ...
  const filteredData = data.filter(item => 
    ...
    (item.id_nhap && item.id_nhap.toLowerCase().includes(searchTerm.toLowerCase()))
  );
    ...
    <div className="min-h-screen bg-[#f1f5f9] font-display text-slate-700">
      ...
      <main className="max-w-full mx-auto p-4 space-y-4">
        ...
        <div className="flex gap-2">
          ...
          >
            <Folder size={18} /> Thanh toán nhập hàng
          </button>
        </div>
