...
import LocTonKhoDialog from './components/LocTonKhoDialog';

interface LocTonKhoData {
  id: string;
  ma_ton_kho: string;
  kho: string;
  san_pham: string;
...
const LocTonKhoPage: React.FC = () => {
  const navigate = useNavigate();
  const [searchTerm, setSearchTerm] = useState('');
  const [currentTime, setCurrentTime] = useState(new Date());
  const [data, setData] = useState<LocTonKhoData[]>([]);
  const [loading, setLoading] = useState(true);
  const [isDialogOpen, setIsDialogOpen] = useState(false);
  const [selectedItem, setSelectedItem] = useState<LocTonKhoData | null>(null);
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      const { error } = await supabase.from('ton_kho').delete().eq('id', id);
      if (error) throw error;
      fetchTonKho();
    } catch (err) {
  ...
  const handleEdit = (item: LocTonKhoData) => {
  ...
  const filteredData = data.filter(item => 
    item.ma_ton_kho.toLowerCase().includes(searchTerm.toLowerCase()) ||
    item.kho.toLowerCase().includes(searchTerm.toLowerCase()) ||
    item.san_pham.toLowerCase().includes(searchTerm.toLowerCase())
  );
    ...
    <div className="min-h-screen bg-[#f1f5f9] font-display text-slate-700">
        ...
        <div className="max-w-full mx-auto px-4 h-[60px] flex items-center justify-between">
          <div className="flex items-center gap-6">
            ...
            <nav className="flex items-center gap-3 text-[13px]">
              ...
              <ChevronRight size={14} className="text-slate-300" />
              <span className="bg-[#2563eb] text-white px-2.5 py-0.5 rounded-md font-medium text-[13px]">Lọc tồn kho</span>
            </nav>
          </div>
      ...
      <main className="max-w-full mx-auto p-4 space-y-4">
        ...
        <div className="flex gap-2">
          ...
          >
            <Folder size={18} /> Danh sách tồn kho theo vị trí
          </button>
        </div>
          ...
          <div className="flex items-center gap-3 flex-1">
            ...
            <div className="relative w-[300px]">
              ...
              <input
                ...
                placeholder="Tìm ID, kho, sản phẩm..."
                value={searchTerm}
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-left border-collapse">
              ...
              <tbody className="divide-y divide-slate-100 italic">
                {loading ? (
                  <tr>
                    <td colSpan={5} className="px-6 py-12 text-center text-slate-400 font-medium italic">Đang tải danh sách tồn kho...</td>
                  </tr>
                ) : filteredData.length === 0 ? (
                  <tr>
                    <td colSpan={5} className="px-6 py-12 text-center text-slate-400 font-medium italic">Chưa có dữ liệu tồn kho nào</td>
                  </tr>
                ) : (
                    ...
                    <tr key={item.id} className="hover:bg-slate-50/80 transition-colors group">
                      ...
                      <td className="px-4 py-4">
                        <div className="flex items-center gap-2">
                           ...
                           <span className="text-slate-400 font-mono font-bold tracking-tighter uppercase">{item.ma_ton_kho}</span>
                        </div>
                      </td>
                      <td className="px-4 py-4">
      ...
      <LocTonKhoDialog 
        ...
        onSuccess={fetchTonKho}
      />
    </div>
  );
};

export default LocTonKhoPage;
