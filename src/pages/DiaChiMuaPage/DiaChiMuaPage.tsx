...
import DiaChiMuaDialog from './components/DiaChiMuaDialog';

interface DiaChiMuaData {
  id: string;
  ma_dia_chi_mua: string;
  dia_chi_mua: string;
}

const DiaChiMuaPage: React.FC = () => {
  const navigate = useNavigate();
  const [searchTerm, setSearchTerm] = useState('');
  const [currentTime, setCurrentTime] = useState(new Date());
  const [data, setData] = useState<DiaChiMuaData[]>([]);
  const [loading, setLoading] = useState(true);
  const [isDialogOpen, setIsDialogOpen] = useState(false);
  const [selectedItem, setSelectedItem] = useState<DiaChiMuaData | null>(null);
  ...
  const fetchAddresses = async () => {
    ...
    try {
      ...
      console.error('Lỗi khi tải danh sách địa chỉ mua:', err);
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      const { error } = await supabase.from('dia_chi_mua').delete().eq('id', id);
      if (error) throw error;
      fetchAddresses();
    } catch (err) {
  ...
  const handleEdit = (item: DiaChiMuaData) => {
  ...
  const filteredData = data.filter(item => 
    item.ma_dia_chi_mua.toLowerCase().includes(searchTerm.toLowerCase()) ||
    item.dia_chi_mua.toLowerCase().includes(searchTerm.toLowerCase())
  );
        ...
        <div className="max-w-full mx-auto px-4 h-[60px] flex items-center justify-between">
          <div className="flex items-center gap-6">
            ...
            <nav className="flex items-center gap-3 text-[13px]">
              ...
              <span className="bg-[#2563eb] text-white px-2.5 py-0.5 rounded-md font-medium text-[13px]">Địa chỉ mua</span>
            </nav>
          </div>
        ...
        <div className="bg-white p-3 rounded-lg border border-slate-200 shadow-sm flex items-center justify-between gap-4">
          <div className="flex items-center gap-3 flex-1">
            ...
            <div className="relative w-[300px]">
              ...
              <input
                type="text"
                className="w-full pl-9 pr-4 py-1.5 border border-slate-200 rounded text-[13px] focus:ring-1 focus:ring-orange-500 focus:border-orange-500 outline-none placeholder-slate-400 transition-all font-medium text-slate-700 shadow-sm"
