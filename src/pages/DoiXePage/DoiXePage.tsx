...
import DoiXeDialog from './components/DoiXeDialog';
...
const DoiXePage: React.FC = () => {
  const navigate = useNavigate();
  const [searchTerm, setSearchTerm] = useState('');
  const [currentTime, setCurrentTime] = useState(new Date());
  const [data, setData] = useState<DoiXeData[]>([]);
  const [loading, setLoading] = useState(true);
  const [isDialogOpen, setIsDialogOpen] = useState(false);
  const [selectedItem, setSelectedItem] = useState<DoiXeData | null>(null);
  const [currentTab, setCurrentTab] = useState<'all'>('all');

  const fetchDoiXe = async () => {
    setLoading(true);
    try {
  ...
  const handleDelete = async (id: string) => {
    if (!window.confirm('Bạn có chắc chắn muốn xóa xe này khỏi danh sách?')) return;
    try {
      const { error } = await supabase.from('doi_xe').delete().eq('id', id);
      if (error) throw error;
      fetchDoiXe();
    } catch (err) {
  ...
  const handleEdit = (item: DoiXeData) => {
  ...
  const filteredData = data.filter(item => 
    ...
    item.ten_tai_xe.toLowerCase().includes(searchTerm.toLowerCase()) ||
    item.so_dien_thoai.toLowerCase().includes(searchTerm.toLowerCase())
  );
    ...
    <div className="min-h-screen bg-[#f1f5f9] font-display text-slate-700">
      ...
      <main className="max-w-full mx-auto p-4 space-y-4">
        ...
        <div className="flex gap-2">
          ...
          >
            <Folder size={18} /> Danh sách xe đợi
          </button>
        </div>
                ...
                {loading ? (
                  ...
                  filteredData.map((item) => (
                    <tr key={item.id} className="hover:bg-slate-50/80 transition-colors group">
                      ...
                      <td className="px-4 py-4 text-[14px] text-slate-900 font-bold text-right tabular-nums">
                        {item.trong_tai} Tấn
                      </td>
                      <td className="px-4 py-4 text-center">
                        <span className={clsx(
                          "px-3 py-1 rounded-full text-[11px] font-bold border",
                          item.tinh_trang === 'Đang lấy hàng' ? "bg-blue-50 text-blue-600 border-blue-100" :
                          item.tinh_trang === 'Đang xếp hàng' ? "bg-amber-50 text-amber-600 border-amber-100" :
                          item.tinh_trang === 'Đã hoàn tất' ? "bg-green-50 text-green-600 border-green-100" :
      ...
      <DoiXeDialog 
        ...
        onSuccess={fetchDoiXe}
      />
    </div>
  );
};

export default DoiXePage;
