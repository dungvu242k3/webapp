...
import CongNoBanDialog from './components/CongNoBanDialog';

interface CongNoBanData {
  id: string;
  id_ref: string;
  so_tien: number;
  thoi_gian: string;
...
const CongNoBanPage: React.FC = () => {
  const navigate = useNavigate();
  const [searchTerm, setSearchTerm] = useState('');
  const [currentTime, setCurrentTime] = useState(new Date());
  const [data, setData] = useState<CongNoBanData[]>([]);
  const [loading, setLoading] = useState(true);
  const [isDialogOpen, setIsDialogOpen] = useState(false);
  const [selectedItem, setSelectedItem] = useState<CongNoBanData | null>(null);
  const [currentTab, setCurrentTab] = useState<'all'>('all');

  const fetchCongNoBan = async () => {
    setLoading(true);
    try {
      ...
      console.error('Lỗi khi tải dữ liệu công nợ bán:', err);
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleDelete = async (id: string) => {
    if (!window.confirm('Bạn có chắc chắn muốn xóa phiếu nợ bán này?')) return;
    try {
      const { error } = await supabase.from('cong_no_ban').delete().eq('id', id);
      if (error) throw error;
      fetchCongNoBan();
    } catch (err) {
  ...
  const handleEdit = (item: CongNoBanData) => {
    ...
    <div className="min-h-screen bg-[#f1f5f9] font-display text-slate-700">
        ...
        <div className="max-w-full mx-auto px-4 h-[60px] flex items-center justify-between">
          <div className="flex items-center gap-6">
            ...
            <nav className="flex items-center gap-3 text-[13px]">
              ...
              <ChevronRight size={14} className="text-slate-300" />
              <span className="bg-[#2563eb] text-white px-2.5 py-0.5 rounded-md font-medium text-[13px]">Công nợ bán</span>
            </nav>
          </div>
      ...
      <main className="max-w-full mx-auto p-4 space-y-4">
        ...
        <div className="flex gap-2">
          ...
          >
            <Folder size={18} /> Tất cả phiếu nợ bán
          </button>
        </div>
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="bg-slate-50 border-b border-slate-200 text-slate-600 text-[12px] font-bold uppercase tracking-wider">
                  ...
                  <th className="px-4 py-3 text-right">Số Tiền Phải Thu</th>
                  <th className="px-4 py-3 text-center">Thời Gian Bán</th>
              ...
              <tbody className="divide-y divide-slate-100 text-[13px]">
                {loading ? (
                  <tr>
                    <td colSpan={6} className="px-6 py-12 text-center text-slate-400 font-medium italic">Đang tải dữ liệu hồ sơ nợ bán...</td>
                  </tr>
                ) : filteredData.length === 0 ? (
                  <tr>
                    <td colSpan={6} className="px-6 py-12 text-center text-slate-400 font-medium italic">Không tìm thấy hồ sơ nợ bán nào</td>
                  </tr>
                ) : (
                  filteredData.map((item) => (
                    <tr key={item.id} className="hover:bg-slate-50/80 transition-colors group">
                      ...
                      <td className="px-4 py-4 text-[14px] text-orange-600 font-extrabold text-right tabular-nums">
      ...
      <CongNoBanDialog 
        ...
        onSuccess={fetchCongNoBan}
      />
    </div>
  );
};

export default CongNoBanPage;
