...
import NhanSuDialog from './components/NhanSuDialog';
...
const NhanSuPage: React.FC = () => {
  const navigate = useNavigate();
  const [searchTerm, setSearchTerm] = useState('');
  const [currentTime, setCurrentTime] = useState(new Date());
  const [data, setData] = useState<NhanSuData[]>([]);
  const [loading, setLoading] = useState(true);
  const [isDialogOpen, setIsDialogOpen] = useState(false);
  const [selectedItem, setSelectedItem] = useState<NhanSuData | null>(null);
  ...
  const handleEdit = (item: NhanSuData) => {
  ...
  const filteredData = data.filter(item => 
    item.ma_nhan_su.toLowerCase().includes(searchTerm.toLowerCase()) ||
    ...
    <div className="min-h-screen bg-[#f1f5f9] font-display text-slate-700">
        ...
        <div className="max-w-full mx-auto px-4 h-[60px] flex items-center justify-between">
          <div className="flex items-center gap-6">
            ...
            <nav className="flex items-center gap-3 text-[13px]">
              ...
              <button className="text-slate-500 hover:text-[#2563eb] transition-colors">Nhân sự</button>
              <ChevronRight size={14} className="text-slate-300" />
              <span className="bg-[#2563eb] text-white px-2.5 py-0.5 rounded-md font-medium text-[13px]">Danh sách nhân viên</span>
            </nav>
          </div>
      ...
      <main className="max-w-full mx-auto p-4 space-y-4">
        ...
        <div className="flex gap-2">
          ...
          >
            <Folder size={18} /> Danh mục nhân sự
          </button>
        </div>
        ...
        <div className="bg-white p-3 rounded-lg border border-slate-200 shadow-sm flex items-center justify-between gap-4">
          <div className="flex items-center gap-3 flex-1">
            ...
            <div className="relative w-[300px]">
              ...
              <input
                ...
                placeholder="Tìm ID, họ tên, email..."
                value={searchTerm}
          ...
          <div className="flex items-center gap-3">
            ...
            <button
              onClick={handleAdd}
              className="bg-yellow-600 hover:bg-yellow-700 text-white px-5 py-1.5 rounded flex items-center gap-2 text-[14px] font-semibold transition-all active:scale-95 shadow-lg shadow-yellow-500/10"
            >
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="bg-slate-50 border-b border-slate-200 text-slate-600 text-[12px] font-bold uppercase tracking-wider">
                  ...
                  <th className="px-4 py-3 w-[150px]">ID Nhân sự</th>
              ...
              <tbody className="divide-y divide-slate-100 text-[13px]">
                {loading ? (
                  <tr>
                    <td colSpan={5} className="px-6 py-12 text-center text-slate-400 font-medium italic">Đang tải danh sách nhân sự...</td>
                  </tr>
                ) : filteredData.length === 0 ? (
                  <tr>
                    <td colSpan={5} className="px-6 py-12 text-center text-slate-400 font-medium italic">Chưa có dữ liệu nhân sự nào</td>
                  </tr>
                ) : (
                  filteredData.map((item) => (
                    <tr key={item.id} className="hover:bg-slate-50/80 transition-colors group">
                      ...
                      <td className="px-4 py-4">
                        <span className="text-slate-400 font-bold uppercase font-mono tracking-tighter">{item.ma_nhan_su}</span>
                      </td>
                      <td className="px-4 py-4">
                        <div className="flex items-center gap-2">
                           <UserCircle size={14} className="text-slate-400" />
                           <span className="text-slate-900 font-bold">{item.ho_ten}</span>
                        </div>
                      </td>
                      <td className="px-4 py-4">
