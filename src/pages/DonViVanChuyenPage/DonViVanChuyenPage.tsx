...
import DonViVanChuyenDialog from './components/DonViVanChuyenDialog';

interface DonViVanChuyenData {
  id: string;
  ma_dvvc: string;
  ten_dvvc: string;
}

const DonViVanChuyenPage: React.FC = () => {
  const navigate = useNavigate();
  const [searchTerm, setSearchTerm] = useState('');
  const [currentTime, setCurrentTime] = useState(new Date());
  const [data, setData] = useState<DonViVanChuyenData[]>([]);
  const [loading, setLoading] = useState(true);
  const [isDialogOpen, setIsDialogOpen] = useState(false);
  const [selectedItem, setSelectedItem] = useState<DonViVanChuyenData | null>(null);
  ...
  const fetchDVVC = async () => {
    ...
    try {
      ...
      console.error('Lỗi khi tải danh sách DVVC:', err);
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      const { error } = await supabase.from('don_vi_van_chuyen').delete().eq('id', id);
      if (error) throw error;
      fetchDVVC();
    } catch (err) {
  ...
  const handleEdit = (item: DonViVanChuyenData) => {
  ...
  const filteredData = data.filter(item => 
    item.ma_dvvc.toLowerCase().includes(searchTerm.toLowerCase()) ||
    item.ten_dvvc.toLowerCase().includes(searchTerm.toLowerCase())
  );
    ...
    <div className="min-h-screen bg-[#f1f5f9] font-display text-slate-700">
      ...
      <main className="max-w-full mx-auto p-4 space-y-4">
        ...
        <div className="flex gap-2">
          ...
          >
            <Folder size={18} /> Danh sách đơn vị vận chuyển
          </button>
        </div>
        ...
        <div className="bg-white p-3 rounded-lg border border-slate-200 shadow-sm flex items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-3">
            ...
            <button
              onClick={handleAdd}
              className="bg-slate-700 hover:bg-slate-800 text-white px-5 py-1.5 rounded flex items-center gap-2 text-[14px] font-semibold transition-all active:scale-95 shadow-lg shadow-slate-500/10"
            >
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-left border-collapse">
              ...
              <tbody className="divide-y divide-slate-100 italic">
                {loading ? (
                  <tr>
                    <td colSpan={4} className="px-6 py-12 text-center text-slate-400 font-medium italic">Đang tải danh sách đơn vị vận chuyển...</td>
                  </tr>
                ) : filteredData.length === 0 ? (
                  <tr>
                    <td colSpan={4} className="px-6 py-12 text-center text-slate-400 font-medium italic">Chưa có đơn vị vận chuyển nào</td>
                  </tr>
                ) : (
                    ...
                    <tr key={item.id} className="hover:bg-slate-50/80 transition-colors group">
                      ...
                      <td className="px-4 py-4">
                        <div className="flex items-center gap-2">
                           ...
                           <span className="text-slate-400 font-mono font-bold tracking-tighter uppercase">{item.ma_dvvc}</span>
                        </div>
                      </td>
                      <td className="px-4 py-4">
                        <div className="flex items-center gap-2.5 text-slate-900 font-bold uppercase">
...
export default DonViVanChuyenPage;
